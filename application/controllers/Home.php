...
class Home extends CI_Controller {
    ...
    public function verificarLogin(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($user){
                ...
                #  $this->encryption->decrypt($user->senha);

                if($password == $password_stored){
                    $session_data = array('nome' => $user->nome, 'email' => $user->email, 'login' => $user->login, 'telefone' => $user->telefone, 'celular' => $user->celular, 'id' => $user->idUsuarios,'permissao' => $user->permissoes_id, 'foto' => '01avatar.png', 'logado' => TRUE);
    ...
    public function reset_pass_enviar_link(){
        ...
        if($email){
            $this->email->from("contato@GB Auto.tecnologia.ws", 'Sistema GB Auto');
            $this->email->subject("Vamos alterar a senha?");
            $this->email->reply_to("contato@GB Auto.tecnologia.ws");
